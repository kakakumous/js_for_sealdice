...
cmdAddItemInfo.help = ".添加道具信息 <名称> <影响变量（例：$m金钱，若为不可使用道具，填写#）> <影响变量参数（=+-*操作符开头，例：+1即为金钱+1，在提前定义了最大值的情况下支持%表达）> <描述> <使用回执> <是否限时（1是0否）> <保持时长(小时 限时为1时需>0)>//仅限骰主使用";
cmdAddItemInfo.solve = (ctx, msg, args) => {
    ...
    if (!name || isNaN(ifStatus) || isNaN(upTime)) {
        seal.replyToSender(ctx, msg, "参数错误。参考：.添加道具信息 <名称> <影响变量（例：$m金钱，若为不可使用道具，填写#）> <影响变量参数（=+-*操作符开头）> <描述> <使用回执> <是否限时（1是0否）> <保持时长(小时 限时为1时需>0)\n最后两个参数不填默认为非状态变更道具");
        return seal.ext.newCmdExecuteResult(true);
    }
